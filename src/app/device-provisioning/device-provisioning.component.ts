...
export class DeviceProvisioningComponent implements OnInit {
  ...
  searchThingAccrossAccount: any = false;
  onAccountChange(id, name) {
    this.accountSelect = name;
    if (name != 'select') {
      this.showpagi = false;
      sessionStorage.setItem('setUserAccount', id)
      this.service.getThingsbyAccount(0, 100, this.searchThingAccrossAccount).subscribe(res => {
        console.log(res)
        this.displayList = res;
      });
    } else {
      sessionStorage.setItem('setUserAccount', 'Radius-PF')
  ...
  getEventList() {
    this.service.getthingList(this.page, this.size, this.sort, this.searchThingAccrossAccount).subscribe(res => {
  ...
  searchresult(name: String, description: String) {
    if (description) {
      this.service.getSearchThingsdesc(name, description, this.searchThingAccrossAccount).subscribe(res => {
        this.displayList = res;
        this.showpagi = false;
      });
    } else {
      this.service.getSearchThings(name, description, this.searchThingAccrossAccount).subscribe(res => {
        this.displayList = res;
        this.showpagi = false;
      });
    }
  ...
  onSearchChange(searchValue: string, serchdescription: String) {
    ...
    if (searchValue || serchdescription) {
      ...
      if (sessionStorage.getItem('setUserAccount') == 'Radius-PF') {
        this.getEventList();
        this.showpagi = true;

      } else {
        this.onAccountChange(sessionStorage.getItem('setUserAccount'), this.accountSelect);
      }
  ...
  toggel(data){
    setTimeout(()=>{
       this.searchThingAccrossAccount = this.test;
      if(this.showpagi){
        this.getEventList();
